...
def _paywall_plans(role):
    """Build subscription plan cards for the paywall page — no nested f-strings."""
    is_student = (role == "student")
    # Free plan card
    free_name  = "Student Free" if is_student else "Free"
    free_feats = "Browse 3 gigs/day · Apply to 1 gig/day" if is_student else "Post 2 gigs/month · Basic analytics"
    free_card  = (
        ...
        '<strong style="font-family:var(--fh);font-size:1rem">' + free_name + '</strong>'
        '<span style="font-weight:800">PKR 0</span></div>'
        '<div style="font-size:.8rem;color:var(--gy);line-height:1.7">' + free_feats + '</div>'
        '</div>'
    )
    # Pro plan card
    pro_name   = "Student Pro" if is_student else "Business Pro"
    pro_price  = "PKR 999/month" if is_student else "PKR 2,999/month"
    pro_feats  = ("Unlimited browsing · Unlimited applications · Priority profile"
                  if is_student else
                  "Unlimited gigs · Priority listing · Applicant filters · Dedicated support")
    pro_card   = (
        '<div style="border:2px solid var(--or);border-radius:14px;padding:1.2rem;margin-bottom:.9rem;background:var(--orl)">'
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.6rem">'
        '<strong style="font-family:var(--fh);font-size:1rem">' + pro_name + ' 🔥</strong>'
        '<span style="font-weight:800;color:var(--or)">' + pro_price + '</span></div>'
        '<div style="font-size:.8rem;color:var(--gy);line-height:1.7">' + pro_feats + '</div>'
        '<form method="POST" action="/subscribe">'
        '<input type="hidden" name="plan" value="pro">'
        '<input type="hidden" name="method" value="easypaisa">'
        '<button type="submit" class="btn bp bbl" style="margin-top:.8rem">Subscribe via EasyPaisa →</button>'
        '</form></div>'
    )
    return free_card + pro_card
...
<div style="padding:1.3rem;background:var(--lt);min-height:calc(100vh - 58px)">
  <div style="background:var(--dk);border-radius:14px;padding:2rem;text-align:center;margin-bottom:1.2rem;color:#fff">
    ...
    <p style="color:#9E9C96;font-size:.85rem;margin-top:.4rem">Upgrade to continue using GigBridge</p>
  </div>
  {_paywall_plans(role)}
  <div style="border:1px solid var(--bd);border-radius:14px;padding:1.2rem;margin-bottom:.9rem;background:var(--wh)">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.6rem">
...
def subscribe():
    ...
    u      = get_user()
    # In production: verify payment with EasyPaisa/JazzCash API here
    # For now: simulate successful payment
    expires = (datetime.utcnow() + timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S")
    amt     = 999 if u["role"] == "student" else 2999
    db = get_db()
    db.execute("UPDATE users SET plan=?,plan_expires=? WHERE id=?", ("active", expires, u["id"]))
    db.execute("INSERT INTO payments (user_id,amount,plan,method) VALUES (?,?,?,?)",
               (u["id"], amt, plan, method))
    db.commit(); db.close()
    flash(f"🎉 Subscription activated! Valid for 30 days. Payment via {method.title()} confirmed.", "success")
    return redirect(url_for("student_home" if u["role"]=="student" else "business_home"))
...
def admin_dashboard():
    ...
    stats = {
        ...
        "subs":      db.execute("SELECT COUNT(*) FROM payments").fetchone()[0],
    ...
    def plan_badge(plan):
        return {"trial":'<span class="badge b-amber">Trial</span>',"active":'<span class="badge b-green">Pro</span>',
                "pro":'<span class="badge b-green">Pro</span>',"admin":'<span class="badge b-blue">Admin</span>'}.get(plan,'<span class="badge b-gray">Free</span>')
    ...
    payments_rows  = "".join(f"<tr><td>{p['email']}</td><td style='color:var(--gn);font-weight:700'>PKR {int(p['amount']):,}</td><td>{p['plan']}</td><td>{p['method'] or '—'}</td><td style='color:var(--gy)'>{p['created_at'][:10]}</td></tr>" for p in payments)
...
<div style="padding:1.2rem;background:var(--lt);min-height:calc(100vh - 58px)">
  ...
  <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:.8rem;margin-bottom:1.8rem">
    <div class="card" style="border:2px solid var(--gn);background:var(--gnl)">
      <div style="font-size:.73rem;font-weight:700;color:var(--gn);letter-spacing:.04em;margin-bottom:.3rem">TOTAL REVENUE</div>
      <div style="font-family:var(--fh);font-size:1.8rem;font-weight:800;color:var(--gn)">PKR {int(stats['revenue']):,}</div>
      <div style="font-size:.76rem;color:var(--gn)">{stats['subs']} subscriptions</div>
    </div>
    <div class="card" style="border:2px solid var(--am);background:var(--aml)">
      <div style="font-size:.73rem;font-weight:700;color:var(--am);letter-spacing:.04em;margin-bottom:.3rem">ON FREE TRIAL</div>
      <div style="font-family:var(--fh);font-size:1.8rem;font-weight:800;color:var(--am)">{stats['trial_students'] + stats['trial_biz']}</div>
      <div style="font-size:.76rem;color:var(--am)">{stats['trial_students']} students · {stats['trial_biz']} businesses</div>
    </div>
  </div>
  ...
  <!-- Payments / Revenue -->
  <div class="st">💳 Subscription Payments</div>
  <div class="card" style="padding:.5rem;overflow-x:auto;margin-bottom:1.5rem">
    {'<table class="adm-table"><thead><tr><th>Email</th><th>Amount</th><th>Plan</th><th>Method</th><th>Date</th></tr></thead><tbody>' + payments_rows + '</tbody></table>' if payments else '<div class="empty" style="padding:1.5rem"><div class="ei">💳</div><p>No payments yet</p></div>'}
  </div>
</div>"""
